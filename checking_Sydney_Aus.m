...
% % Path/row-089/083- Sydney, Australia
...
for ROI=1:4
  ...
  if ROI==1
    ...
    UL_x=348445;
    UL_y=6372183-equator;
    UR_x=348728;
    UR_y=6372289-equator;
    LR_x=348863;
    LR_y=6371982-equator;
    LL_x=348599;
    LL_y=6371876-equator;

        elseif ROI==2

    UL_x=345311;
    UL_y=6369830-equator;
    UR_x=345443;
    UR_y=6369806-equator;
    LR_x=345444;
    LR_y=6369765-equator;
    LL_x=345312;
    LL_y=6369792-equator;


       elseif ROI==3

    UL_x=352141;
    UL_y=6381218-equator;
    UR_x=352252;
    UR_y=6381215-equator;
    LR_x=352224;
    LR_y=6381106-equator;
    LL_x=352128;
    LL_y=6381107-equator;

      elseif ROI==4

        % ROI Coordinates
    UL_x=340605;
    UL_y=6373907-equator;
    UR_x=340819;
    UR_y=6373880-equator;
    LR_x=340809;
    LR_y=6373816-equator;
    LL_x=340602;
    LL_y=6373845-equator;
...
MData_file=strcat(base, filesep, dates.name, filesep, '20181120_002253_32_1047_3B_AnalyticMS_metadata.xml');
...
for D_band=1:4 %dove band
    ...
    %Dove_image_file= strcat(base,filesep, dates.name, filesep, '002253_32_cubicspline.tif');
...
%         y = decimate(SNR_esti_D,4);
    ...
    SNR_esti_D=(SNR_esti(~isnan(SNR_esti))); 
    close all
    plot(MeanTOA_rad_D, SNR_esti_D, 'ko')
...
%     all_mean.L8.ROI(ROI,:) = mean_L8_TOArad;
     ...
     all_mean_S_Aus.D.ROI(ROI,:) = mean_D_TOArad;
     all_mean_S_Aus.L8.ROI(ROI,:) = mean_L8_TOArad;
    % all_mean.L8_2.ROI(ROI,:)= mean_L8_2_TOArad;
   
     all_mean_ref_S_Aus.D.ROI(ROI,:) = mean_D_TOAref;
     all_mean_ref_S_Aus.L8.ROI(ROI,:) = mean_L8_TOAref;
     
     %Standard Deviation- Radiance
     all_SD_S_Aus.D.ROI(ROI,:)= Std_D_TOArad;
     all_SD_S_Aus.L8.ROI(ROI,:)= Std_L8_TOArad;
     
     %Standard Deviation- Reflectance
     all_SD_ref_S_Aus.D.ROI(ROI,:)= Std_D_TOAref;
     all_SD_ref_S_Aus.L8.ROI(ROI,:)= Std_L8_TOAref;
     
     %Relative Standard Deviation from Radiance
     all_RSD_S_Aus.D.ROI(ROI,:) = Std_D_TOArad./mean_D_TOArad;
     all_RSD_S_Aus.L8.ROI(ROI,:) = Std_L8_TOArad./mean_L8_TOArad;
     
     %Relative Standard Deviation from Reflectance
     all_RSD_ref_S_Aus.D.ROI(ROI,:) = Std_D_TOAref./mean_D_TOAref;
     all_RSD_ref_S_Aus.L8.ROI(ROI,:) = Std_L8_TOAref./mean_L8_TOAref; 
end
    all_mean.Radiance.l_S_Aus = all_mean_S_Aus;
    all_mean.Reflectance.l_S_Aus = all_mean_ref_S_Aus;
    
    all_SD.Radiance.l_S_Aus = all_SD_S_Aus;
    all_SD.Reflectance.l_S_Aus = all_SD_ref_S_Aus;
    
    all_RSD.Radiance.l_S_Aus = all_RSD_S_Aus;
    all_RSD.Reflectance.l_S_Aus = all_RSD_ref_S_Aus;

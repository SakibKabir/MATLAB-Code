...
%cd('Y:\Libya 1')
if ispc
    location='Z:\ImageDrive\Sentinel\MSI\P039\R037';
elseif isunix
...
%%Algodones dunes
lat1=[680310 681840  688680 686640];
lon1=[3646740 3648030  3639870 3638490];

lat=[600000 709800 600000 709800 600000];
lon=[3700020 3700020 3590220 3590220 3700020];

SBAF=[1.0014 0.9777 0.9995 0.9847 0.9997 0.9979 0.9982];
...
Reflactance=sum(sum(double(Image)))/(row*column-Field_data)/Q_value;
 ...
 VAA_ROI(i)=mean2(View_Azimuth(View_Azimuth~=0));
  ...
  for i=1:1:7
     figure(i)
     plot(Refl(:,i),'LineStyle','none','Marker','o')
%      
     UNC(i)=std(Refl(:,i))*100./mean(Refl(:,i))
...
db=xmlread('Z:\ImageDrive\Sentinel\MSI\P181\R040\20150802\metadata.xml');
% 
% for i=1:1:13

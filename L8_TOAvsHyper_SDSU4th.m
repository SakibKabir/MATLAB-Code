...
for ref = 1:size(L8_allMean) % all the reflectances inside the ROI
 ...
 for NoPro = 1:size(Hyper_Spectrum_all, 2)
        ...
        HyB_L8_m = HyB_L8.*M_vegi; clear HyB_L8
...
%         end; clear HyB_L8
        ...
        if isnan(M_vegi)
            SSE(1, NoPro) = nan; % In some columns, hyperspectral Data are missing
        else
...
%         end; clear M_sand HyB_L8s
        ...
        SSE(2, NoPro) = sse(OLI_ref, HyB_L8_m);  
       else
        
       end; clear Labels_L8_Sand HyBanded_L8_sand OLI_ref_m HyB_L8_m 
 end; clear NoPro       
    
SSE(SSE==0) = nan; [r, c]= find(SSE == min(min(SSE))); clear SSE
%% Calculating SBAF with appropriate profile
if r(1) == 1 % Vegi
...
elseif r(1) == 2 % Sand
    ...
    Hyper_WL = Hy_WL_Sand;
     ...
     % Storing the appropiate profile
     Hyper_Aprof(ref,:) = {Hyper};
    %%% Banding Reference Sensor OLI- Satnum = 24
    ...
    %%% Store the SSE of OLI and Hyper Banded Reflectance
     ...
     HyB_L8_m = HyBanded_OLIRef.*M; clear M
...
%         end; clear M
     Hy_OLI_banded(ref,:) = HyBanded_OLIRef;
     SSE_all_noscaling(ref) = sse(OLI_ref, HyBanded_OLIRef);
     SSE_all(ref) = sse(OLI_ref, HyB_L8_m); clear OLI_ref HyB_L8_m
end ; clear ref OLI_ref HyBanded_OLIRef Hyper_WLenNM Hyper_Spectrum Hy_WL_Sand Hyper_Spectrum_all L8_allMean

save('SBAF_L8_D1047_SCalSite4.mat'); % This file will have 5 variables in it
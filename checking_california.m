...
%         % ROI Coordinates
%         UL_x=368600;
%         UL_y=6198550-equator;
%         UR_x=369066;
%         UR_y=6198570-equator;
%         LR_x=368998;
%         LR_y=6198356-equator;
%         LL_x=368778;
%         LL_y=6198412-equator;
...
for D_band=1:4 %dove band
    ...
    %Dove_image_file= strcat(base,filesep, dates.name, filesep, '172655_38_cubicspline.tif');
    Dove_image_file= strcat(base,filesep, dates.name, filesep, '20181223_172655_38_1047_3B_AnalyticMS.tif');
...
%     end
    ...
    Std_L8_TOAref(D_band)= std(temp_L8_ref);
     ...
     %Mean TOA Radiance
     all_mean_california.D.ROI(ROI,:) = mean_D_TOArad;
     all_mean_california.L8.ROI(ROI,:) = mean_L8_TOArad;

     %mean TOA Reflectance
     all_mean_ref_california.D.ROI(ROI,:) = mean_D_TOAref;
     all_mean_ref_california.L8.ROI(ROI,:) = mean_L8_TOAref; 
     
     %Standard Deviation- Radiance
     all_SD_california.D.ROI(ROI,:)= Std_D_TOArad;
     all_SD_california.L8.ROI(ROI,:)= Std_L8_TOArad;
     
     %Standard Deviation- Reflectance
     all_SD_ref_california.D.ROI(ROI,:)= Std_D_TOAref;
     all_SD_ref_california.L8.ROI(ROI,:)= Std_L8_TOAref;
     
     %Relative Standard Deviation
     all_RSD_california.D.ROI(ROI,:) = Std_D_TOArad./mean_D_TOArad;
     all_RSD_california.L8.ROI(ROI,:) = Std_L8_TOArad./mean_L8_TOArad;
     
     %Relative Standard Deviation
     all_RSD_ref_california.D.ROI(ROI,:) = Std_D_TOAref./mean_D_TOAref;
     all_RSD_ref_california.L8.ROI(ROI,:) = Std_L8_TOAref./mean_L8_TOAref; 
end

    all_mean.Radiance.a_california = all_mean_california;
    all_mean.Reflectance.a_california = all_mean_ref_california;

    all_SD.Radiance.a_california = all_SD_california;
    all_SD.Reflectance.a_california = all_SD_ref_california;

    all_RSD.Radiance.a_california = all_RSD_california;
    all_RSD.Reflectance.a_california = all_RSD_ref_california;
...
%         hold on
        L8_band2 = all_mean_california.L8.ROI(:,band);
        D_band1 = all_mean_california.D.ROI(:,band);
        D_band1_RSD= all_RSD_california.D.ROI(:,band);
        
        L8_band2_ref = all_mean_california.L8.ROI(:,band);
        D_band1_ref = all_mean_california.D.ROI(:,band);
        
        
       % figure(1)
        hold on
        sz = 120;
        for k=1:length(all_mean_california.L8.ROI(:,band))
        ...
        plot( L8_band2, D_band1, 'color', band_colors{band}, 'LineStyle','None','Marker','.','markers', 18)
...
%         All_Bias.Bias_ci_band1(l,:) = Bias_ci_band1;
        ...
        hold on
        error = D_band1_RSD;
        errorbar(L8_band2, D_band1, error/2, 'oc','LineWidth',1)
